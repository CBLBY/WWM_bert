...
class Instructor:
        ...
        for epoch in range(self.opt.num_epoch):
            ...
            if val_f1 > max_f1:
                ...
                if not os.path.exists('state_dict'):
                    os.mkdir('state_dict')
                path = 'state_dict/{0}_val_f1_{1}'.format(self.opt.model_name, round(val_f1, 4))
    ...
    def _do_test(self, dataloader):
        self.model.eval()
        t_outputs_all, t_guid_all = None, None
        with torch.no_grad():
            for i_batch, sample_batch in enumerate(dataloader):
                t_inputs = [sample_batch[col].to(self.opt.device) for col in self.opt.inputs_cols]
                t_guid = np.asarray(sample_batch['guid'])
                t_outputs = self.model(t_inputs)
    ...
    def run(self):
        ...
        self.model.eval()
        self._do_test(test_data_loader)
